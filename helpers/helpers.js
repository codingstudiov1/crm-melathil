...
module.exports = {
  createUser: (userData) => {
    return new Promise((resolve, reject) => {
      instantHelper
        ...
        .then(async () => {
          let userId = await instantHelper.generateMemberId();
  ...
  getUserLogin: (userInfo) => {
    return new Promise((resolve, reject) => {
      Users.aggregate([
        ...
        console.log(result[0]?.usertype?.permissions);
        if (result[0]) {
          switch (result[0].status) {
            case Constants.PENDING_STATUS: {
              reject({
                status: false,
                message:
                  "Your verification is pending..Kindly wait for a while",
              });
            }
            ...
            case Constants.RESIGN_STATUS: {
              reject({
                status: false,
                message:
                  "Currently you have no permission to access the website",
              });
            }
  ...
  approveEmployee: (id) => {
    return new Promise((resolve, reject) => {
      Users.updateOne({ employeeId: id }, { status: ACTIVE_STATUS }).then(
        (user) => {
          console.log(user);
          resolve();
        }
  ...
  rejectEmployee: (id) => {
    return new Promise((resolve, reject) => {
      Users.updateOne({ employeeId: id }, { status: REJECT_STATUS }).then(
        (user) => {
          console.log(user);
          resolve();
        }
